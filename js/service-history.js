document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!soldierCalendarsContainer || soldierCalendarsContainer.children.length === 0 || !services || services.length === 0) {
    ...
    noSoldiersServicesMessage.style.display = "block";
  } else {
    document.querySelector(".history-section-container").style.display = "block";
  }
  ...
  function renderSoldierCalendar(soldier) {
    ...
    const defaultDates = serviceHistoryFlags
      .map((value, index) => {
        if (value === 1) {
  ...
  if (soldiers && soldiers.length > 0) {
    ...
    soldierCalendarsContainer.textContent = "Δεν υπάρχουν καταχωρημένοι οπλίτες.";
  }
});
